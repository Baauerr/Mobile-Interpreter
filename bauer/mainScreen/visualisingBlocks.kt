...
import com.example.scratch.mathOperations.GlobalStack
        ...
        Row() {
            Box(modifier = Modifier.weight(10f))
            {
                when (block.blockType){
                    "createVariable" -> textFieldWithMapValue(block = block)
                "mathOperation" -> opsExpression(block = block)
                "createPrint" -> printBlock( block = block)
                "createConditions" -> conditions(block = block)
                }
            }
            Button(
                modifier = Modifier
