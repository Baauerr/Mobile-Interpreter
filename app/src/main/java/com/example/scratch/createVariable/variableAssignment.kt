...
fun textFieldWithMapValue(block: Blocks) {
    ...
    Column(
        modifier = Modifier
            .padding(top = columnPadding / 2, start = columnPadding, end = columnPadding)
    ) {
        ...
        Text (text = "Variable",  modifier = Modifier.padding(top = textPadding, bottom = textPadding), fontFamily = forHeaders)
    }
}